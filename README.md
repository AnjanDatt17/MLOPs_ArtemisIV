...
- **Project Lead:** Your name (or your team name) (your.email@depaul.edu)
- **Team Members:** *To be filled in*

## Project Overview

Movie Recommendation is a machine learning project that implements Recommending movies to users.
...
This template uses the modern **`src/` layout** — the importable package lives in `src/teamartemisIV/`, decoupled from the repository root. That forces `pip install -e .` before imports work, which catches packaging bugs early.

```
teamartemisIV/                  # Repository root
├── src/
│   └── teamartemisIV/          # Importable Python package
...
Data and model artifacts are accessed via the constants in `teamartemisIV.config` (`PROJECT_ROOT`, `DATA_DIR`, `MODELS_DIR`, …) rather than relative paths — code is independent of where you invoke it from.
